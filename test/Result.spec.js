...
describe('<Result/>', () => {
  ...
  it('calculates the amount of pizza required', () => {
    const wrapper = shallow(<Result
      pizzaOptions={{ bogof: false }}
      people={6}
      slices={7}
      />);
    const {solution, slicesAvailible} = wrapper.instance()._calculateSolution(42);
    expect(solution).to.equal("1 Larges, 4 Mediums");
    expect(slicesAvailible).to.equal(42);
    expect(wrapper.find('#pizzas-required').text()).to.equal("1 Larges, 4 Mediums");
  ...
  it('calculates the amount of pizza required with a bogof offer', () => {
    const wrapper = shallow(<Result
      pizzaOptions={{ bogof: true }}
      people={6}
      slices={7}
      />);
    const {solution, slicesAvailible} = wrapper.instance()._calculateSolution(42, { bogof: true });
